    ...
    if (strcmp(type, "admin") == 0) {
        ...
        do {
            ...
            case 2:
                ...
                if (fichier == NULL) {
                    fprintf(stderr, "(serveur) Erreur lors de l'ouverture du fichier contacts.txt\n");
                    rep = -1; // Indicateur d'erreur
                    send(connectedSocketFD, &rep, sizeof(rep), 0);
                } else {
                    while (fscanf(fichier, "%s %s %s %s %s %s %s", c.nom, c.prenom, c.GSM, c.email, c.adresse.rue, c.adresse.ville, c.adresse.pays) == 7) {
                        // Envoyer chaque contact au client
                        send(connectedSocketFD, &c, sizeof(c), 0);
                    }
                    fclose(fichier);

                    // Envoyer un signal de fin au client
                    rep = 0; // Indicateur de fin
                    send(connectedSocketFD, &rep, sizeof(rep), 0);
                    printf("\n\t---> Tous les contacts ont été affichés avec succès !!\n");
                }
                break;
